...
import type { Viewer } from "@/lib/identity/resolve-viewer";
...
// never pulls in lib/identity/viewer.ts's React.cache() wiring, which only works
// inside Next's bundler and throws in a plain Node/test environment.
async function realDeps(): Promise<WithAuthDeps> {
  const { getViewer } = await import("@/lib/identity/viewer");
